...
function init(that, barHeight, keys, isShowView,isShowKey, isShowHis, callBack) {
    var temData = {};
    var view = {
        barHeight: barHeight,
        isShow: isShowView
...
function getHisKeys(that) {
    var value = [];
    try {
        value = wx.getStorageSync('wxSearchHisKeysTM')
        if (value) {
...
function wxSearchAddHisKey(that) {
    ...
    var value = wx.getStorageSync('wxSearchHisKeysTM');
    if(value){
        if(value.indexOf(text) < 0){
            value.unshift(text);
        }
        wx.setStorage({
            key:"wxSearchHisKeysTM",
            data:value,
            success: function(){
                getHisKeys(that);
            }
