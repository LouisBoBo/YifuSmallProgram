...
Page({
  ...
  onLoad: function(options) {
    ...
    //预售时间
    if (options.delivery_time != undefined)
    {
      this.setData({
  ...
  submitOrder: function(e) {
    ...
    } else if (this.data.buyType == 10 || this.data.buyType == 9) //1元订单
    {
      ...
      else if (vip_free == 1) { //免费领
        var free_order_channel = wx.getStorageSync("v_type_channel");
        if (free_order_channel != undefined && free_order_channel != '')
        {
          free_order_channel = free_order_channel.substring(7, free_order_channel.length);
        }else{
          free_order_channel = '';
        }
        dataUrl = config.Host + "order/addOrderVipFreeBuy?" +
          config.Version +
          "&token=" + app.globalData.user.userToken +
          '&message=' + message +
          '&result=' + result +
          '&shop_code=' + this.data.shopData.shopCode +
          '&page3=' + ((shouYePage == 'ThreePage' || shouYePage == 'FivePage') ? 0 : 0) +
          '&address_id=' + this.data.address_id +
          ...
          '&vip_type=' + this.data.vip_type +
          '&first_diamond=' + app.globalData.first_diamond +
          "&order_channel=" + free_order_channel
      }
      var that = this;
      var ids = '';
  ...
  confirmorderResult: function(data) {
    ...
    if (data.status == 1) {
      ...
      //   var isVip = data.isVip != undefined ? data.isVip:'0';

      //   //2020-2-15 最新修改
      //   if (isVip > 0 && isVip !=3)//是会员
      //   { 
      ...
      //     })
      //   }else{
      //     wx.setStorageSync('oneYuan_order_code', data.order_code);
      //     wx.redirectTo({
      //       url: "/pages/listHome/order/oneBuyLuckPan/oneBuyLuckPanIOS?" + "order_code=" + data.order_code,
      //     })
      //   }
      ...
      if (this.data.buyType == 1) {
        ...
        // 2020-3-18 何波最新修改
        var jumpDrawPage = data.jumpDrawPage;//0 跳转抽奖  1待发货
        if (vip_free == 1)
        {
          if (jumpDrawPage == 0) {
            wx.setStorageSync('oneYuan_order_code', data.order_code);
